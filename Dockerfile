...
RUN echo 'server { listen 3006; server_name _; root /usr/share/nginx/html; index index.html; absolute_redirect off; \
    location = /pdv/assets/remoteEntry.js { alias /usr/share/nginx/html/assets/remoteEntry.js; add_header Content-Type "application/javascript"; add_header Cache-Control "no-cache"; add_header CDN-Cache-Control "no-store"; add_header Access-Control-Allow-Origin "*"; } \
    location = /assets/remoteEntry.js { alias /usr/share/nginx/html/assets/remoteEntry.js; add_header Content-Type "application/javascript"; add_header Cache-Control "no-cache"; add_header CDN-Cache-Control "no-store"; add_header Access-Control-Allow-Origin "*"; } \
...
RUN mkdir -p /etc/nginx/snippets && \
    ...
    sed -i 's/server {/server { server_tokens off; include \/etc\/nginx\/snippets\/security-headers.conf;/' /etc/nginx/conf.d/default.conf
